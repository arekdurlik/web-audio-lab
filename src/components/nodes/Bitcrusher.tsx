...
export function Bitcrusher({ id, data }: BitcrusherProps) {
  ...
  const instance = useRef(new AudioWorkletNode(audio.context, 'bit-crusher-processor', {
    parameterData: { bitDepth: params.bitDepth, frequencyReduction: params.sampleRateReduction }
  }))
  ...
  function handleBitDepth(value: number) {
    ...
    instance.current.parameters.get('bitDepth')?.setValueAtTime(value, audio.context.currentTime)
  }

  function handleSampleRateReduction(value: number) {
    setParams(state => ({ ...state, sampleRateReduction: value }))
    instance.current.parameters.get('frequencyReduction')?.setValueAtTime(value, audio.context.currentTime)
  }
  ...
  const Parameters = 
    <FlexContainer direction='column'>
      <RangeInput
        ...
        onExpandChange={v => setParams(state => ({ ...state, expanded: { ...state.expanded, b: v }}))}
      />
      <Hr/>
      <RangeInput
