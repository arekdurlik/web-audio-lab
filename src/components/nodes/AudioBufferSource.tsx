...
export function AudioBufferSource({ id, data }: AudioBufferSourceProps) {
    ...
    async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
        ...
        setLoadedFiles(
            state =>
                new Map([...state.entries(), [file.name, { name: file.name, buffer: audioBuffer }]])
        );
    ...
    const Parameters = (
        <FlexContainer direction="column">
            <FlexContainer align="center">
                <PlayButton
                    onClick={() => {
                        setParams(state => ({ ...state, playing: !state.playing }));
            ...
            <RangeInput
                ...
                onExpandChange={v =>
                    setParams(state => ({ ...state, expanded: { ...state.expanded, pr: v } }))
                }
            />
        </FlexContainer>
    );
