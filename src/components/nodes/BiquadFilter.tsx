...
export function BiquadFilter({ id, data }: BiquadFilterProps) {
    const [params, setParams] = useState<BiquadFilterParams>({
        ...{
            ...
            Q: 0,
            gain: 0,
            expanded: { t: true, f: true, d: true, q: true, g: true },
        },
        ...data.params,
    });
    ...
    const Parameters = (
        <FlexContainer direction="column">
            <SelectInput
                ...
                onExpandChange={t =>
                    setParams(state => ({ ...state, expanded: { ...state.expanded, t } }))
                }
            />
            <Hr />
            <RangeInput
                ...
                onExpandChange={f =>
                    setParams(state => ({ ...state, expanded: { ...state.expanded, f } }))
                }
            />
            <Hr />
            <RangeInput
                ...
                onExpandChange={d =>
                    setParams(state => ({ ...state, expanded: { ...state.expanded, d } }))
                }
            />
            <Hr />
            <RangeInput
                ...
                onExpandChange={q =>
                    setParams(state => ({ ...state, expanded: { ...state.expanded, q } }))
                }
            />
            <Hr />
            <RangeInput
                ...
                numberInput
                numberInputWidth={NUMBER_INPUT_WIDTH}
                disabled={['lowpass', 'highpass', 'bandpass', 'notch', 'allpass'].includes(
                    params.type
                )}
                expanded={params.expanded.g}
                onExpandChange={g =>
                    setParams(state => ({ ...state, expanded: { ...state.expanded, g } }))
                }
            />
        </FlexContainer>
    );
