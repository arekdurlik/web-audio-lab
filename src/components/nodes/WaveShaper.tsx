...
export function WaveShaper({ id, data }: WaveShaperProps) {
    const [params, setParams] = useState<WaveShaperParams>({
        ...{
            type: 'array',
            array: '-1,0,1',
            equation: '(3 + 20) * x * 57 * (Math.PI / 180) / (Math.PI + 20 * Math.abs(x))',
            oversample: 'none',
            expanded: { o: true, t: true },
        },
        ...data.params,
    });
    ...
    const Parameters = (
        <FlexContainer direction="column">
            <SelectInput
                ...
                onChange={e =>
                    setParams(state => ({ ...state, oversample: e.target.value as OverSampleType }))
                }
                options={[
            ...
            <SelectInput
                ...
                value={params.type}
                onChange={e =>
                    setParams(state => ({ ...state, type: e.target.value as WaveShaperType }))
                }
                options={[
