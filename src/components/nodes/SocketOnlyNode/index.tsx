...
import { HoverOptions, LeftOptions } from '../BaseNode/styled';
...
export function SocketOnly({ id, data, defaultRotation = 0, label, socket }: SocketOnlyProps) {
    ...
    return (
        <div onPointerOver={() => setActive(true)} onPointerOut={() => setActive(false)}>
            <Wrapper>
                {active && (
                ...
                <CircleHandle rotation={rotation} position={positions[rotation][2]} {...socket} />
            </Wrapper>
        </div>
    );
}
