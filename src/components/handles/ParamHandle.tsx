...
export function ParamHandle({ id, label, type, position, offset = 20, tooltip }: HandleProps) {
    ...
    const points = (function () {
        switch (direction) {
            case 'left':
                return '0.5,5.5  7,-1  7,11';
            case 'top':
                return '5.5,0.5  -1,7  11,7';
            case 'right':
                return '1,0      1,10  6.5,5.5';
            case 'bottom':
                return '5,6  0,1   10,1';
        }
    })();
    ...
    const Handle = (
        <StyledHandle id={id} type={type} direction={direction} position={position} offset={offset}>
            <Triangle direction={direction} src={param}></Triangle>
            <Label position={position}>{label}</Label>
        </StyledHandle>
    );

    return <>{tooltip ? <Tooltip content={tooltip}>{Handle}</Tooltip> : Handle}</>;
}
