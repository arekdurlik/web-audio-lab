...
import { Edge, useViewport } from 'reactflow';
...
export function EdgeController({ edges }: { edges: Edge[] }) {
    const [edgeWrapper, setEdgeWrapper] = useState({
    ...
    useEffect(() => {
        ...
        setMutationObserver(
            new MutationObserver(function moDispatchCallback(entries) {
...
function BezierEdge({
    ...
    edgeWrapper: { el: HTMLDivElement; x: number; y: number };
}) {
    ...
    async function setupCanvas() {
        const el = await waitForElement(
            `.react-flow__edge-default[data-testid="rf__edge-${edge.id}"]`
        );
        if (!el) return;
